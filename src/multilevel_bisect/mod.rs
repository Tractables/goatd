...
//!
//! Its callers — the nested-dissection elimination order, the refinement pass
//! — want a small separator rather than a small edge cut, a distinction
//! `multilevel_bisect` acts on.
//!
//! Sibling of [`multilevel_hg_bisect`](crate::multilevel_hg_bisect), the
...
/// Multilevel 2-way bisection of the graph `edges` describes over `n`
/// vertices: one pass, refined by V-cycles. Returns one side per vertex, `0`
/// or `1`, `n` long; both sides are non-empty for any `n >= 2`.
///
/// `max_imbalance` bounds each side at `(0.5 + max_imbalance)` of the
/// vertices. `base_seed` selects the RNG stream the pass runs on; nothing here
/// reads a clock, so one seed gives one bisection.
///
/// One pass and not a best-of-N over restarts, for the reason under "Where the
/// two bisectors differ" in `fm_common`.
pub fn multilevel_bisect(
    ...
    let mut rng = bisector_stream(crate::bisect_seed::restart_seed(base_seed, 0));
