//! The elimination skeleton the deterministic greedy orders share.
//!
...
pub(super) enum AfterElim {
    ...
    EnterCheapMode,
    /// The residual is too large to finish even in cheap mode. The caller may
    /// complete it directly with a path order.
    Bail,
}

/// The parts of a greedy heap elimination that differ per core.
///
...
///
/// Only [`pop`](Self::pop), [`push`](Self::push),
/// [`live_score`](Self::live_score) and [`queue_len`](Self::queue_len) have to
/// be written. The defaults below are the simplest core there is — seed the
...
pub(super) trait ElimPolicy {
    ...
    /// How many entries the queue holds. The probe reports it; nothing in
...
/// [`ElimExit::WidthLimitExceeded`] rather than pay for a decomposition nobody will
/// pick.
...
fn eliminate_greedy_probed<P: ElimPolicy>(
    ...
    loop {
        ...
        if probe.on() {
            let queue_len = policy.queue_len();
            probe.eliminated(graph, &nbrs_buf, bag_len, cheap_mode, queue_len);
        }
        let mark = probe.mark();
        sink.record(v, bag);
