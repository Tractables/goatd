...
///
/// The two sampling orders carry the per-vertex weight they sample tie sets
/// with, so naming one of them and having a weight to hand are the same act.
/// The weight has one entry per vertex of the graph the order is run over;
/// a vertex with a smaller weight is more likely to be drawn from a tie set,
/// so it tends to be eliminated earlier. A uniform weight makes the draw
/// uniform.
...
pub enum Config<'a> {
    ...
    /// ties broken by weighted sampling from the full min-fill tie set. The
    /// shape is `htd::MinFillOrderingAlgorithm`'s.
    MinFillSampled {
        /// Per-vertex weight, one entry per vertex of the graph this order is
        /// run over.
        weight: &'a [u32],
    },
    /// htd-style min-degree: priority = degree only, ties broken by the same
    /// weighted sampling from the tie set.
    MinDegreeSampled {
        /// Per-vertex weight, as [`Config::MinFillSampled`] carries it.
        weight: &'a [u32],
    },
}
...
impl<'a> Config<'a> {
    ...
    pub(super) fn with_tie_weight<'b>(self, weight: &'b [u32]) -> Config<'b> {
        match self {
            ...
            Config::NestedDissection => Config::NestedDissection,
            Config::MinFillSampled { .. } => Config::MinFillSampled { weight },
            Config::MinDegreeSampled { .. } => Config::MinDegreeSampled { weight },
        }
    }
}
...
pub(super) fn run_config_prebuilt(prebuilt: &Prebuilt, spec: RunSpec<'_>) -> ConfigRun {
    ...
    run_config_on_reduced(reduced, single_component_bitset, initial_fill, spec)
}
...
fn run_elimination_raw(
    ...
    let exit = match spec.config {
        ...
        Config::MinDegreeSampled { weight } => {
            let sw = to_sample_weight(weight);
...
fn run_config_per_component(
    ...
    ConfigRun {
        td: build_td_from_steps(all_bags, &global_rank),
        exit: combined_exit,
    }
}
...
pub(super) fn run_config_on_reduced(
    ...
    if !single_component_bitset {
        ...
        if components.len() > 1 {
            return run_config_per_component(reduced, components, &salt, spec);
        }
    }
...
fn finalize(steps: ElimSteps, n: usize, exit: ElimExit) -> ConfigRun {
    let mut rank = vec![u32::MAX; n];
    for (v, r) in steps.rank_pairs {
        rank[v as usize] = r as u32;
    }
    ConfigRun {
        td: build_td_from_steps(steps.bags, &rank),
        exit,
    }
}
...
/// convention weight (higher = more likely to sample), so a `P(v) ∝ w+1`
/// sampler eliminates the low-weight vertices first.
...
///   slot 0: MinDegreeSampled — htd-style min-degree: degree-only priority
///                               with weighted random sampling from the
///                               tie set, O(deg·log n) per step
///   slot 1: NestedDissection — O(n log n) hierarchical, fast on dense graphs
///   slot 2: MinFillSampled   — htd-style min-fill: fill-only priority with
///                               weighted random sampling from the tie set
///   slot 3: MinDegreeSampled+42 — second seed for diversity
///   slot 4: NestedDissection+42 — second seed for diversity
///
...
pub(super) fn five_slot_portfolio(base_seed: u64, weight: &[u32]) -> Vec<(Config<'_>, u64)> {
    vec![
        ...
        (Config::MinFillSampled { weight }, base_seed),
        (
            Config::MinDegreeSampled { weight },
            base_seed.wrapping_add(42),
        ),
        (Config::NestedDissection, base_seed.wrapping_add(42)),
    ]
}
