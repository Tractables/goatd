...
//!
//! Motivation: the sampled min-fill elimination is strong locally — each
...
//! structured graphs with a small top-level separator.
//!
//! This module injects FlowCutter's notion of global structure into an
//! elimination TD by (a) picking one balanced separator `S` on the active
//! subgraph, (b) projecting the existing TD onto each side (preserving RIP),
//! (c) gluing the two projections at a new bag = `S`, and (d) recursing on
//! each side while the refinement strictly reduces `(width, total_bag_size)`.
...
use crate::restrict_to_subset;
use crate::td_ops::{glue_at_separator, project_td_keeping_global_ids};
...
/// uninterruptible search can run for seconds. The result is never worse
...
fn refine_inner(
    ...
    let td_a = match project_td_keeping_global_ids(&td, &keep_a) {
        Some(t) => t,
        None => return td,
    };
    let td_b = match project_td_keeping_global_ids(&td, &keep_b) {
        Some(t) => t,
        None => return td,
    };
    let glued = match glue_at_separator(td_a.clone(), td_b.clone(), &sep_global) {
        Some(g) => g,
        None => return td,
    };
