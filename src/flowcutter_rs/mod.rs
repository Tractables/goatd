...
//! [`flowcutter_compute_separator`] wraps it with the two sides a caller
//! splitting a graph needs. Full tree-decomposition construction still crosses
//! into C++ (multilevel recursion, cell→bag conversion, greedy elimination
//! orders) via FFI in [`crate::flowcutter`]; the two are separate halves of
//! FlowCutter, not two implementations of one thing.
//!
...
pub use separator::{FcSeparatorResult, flowcutter_compute_separator};
