...
#include <chrono>  // goatd: wall-clock abandonment (see below)
#include <cstdint>  // goatd: int64_t — not guaranteed transitively on all stdlibs

// goatd: both passes accept an optional ABANDONMENT DEADLINE.
//
...
// completion.
//
// goatd: both passes also accept an optional TOUCH BUDGET, counted in graph
...
// goatd: read and reset the touch counter the two passes accumulate into. The
