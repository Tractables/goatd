...
// goatd: graph elements the two greedy pre-orderings sweep, counted in the same
...
// goatd: has the abandonment deadline passed? `time_point::max()` is the "no
...
// goatd: has this pass spent its touch budget? The deadline above abandons a
...
ArrayIDIDFunc compute_greedy_min_degree_order(
	...
	while(!q.empty()){
		// goatd: abandon WHOLE, never partial — a prefix of an elimination
...
ArrayIDIDFunc compute_greedy_min_shortcut_order(
	...
	for(int x=0; x<node_count; ++x){
		// goatd: the priming loop is itself O(sum of deg^2) and on a dense
	...
	while(!q.empty()){
		// goatd: abandon WHOLE, never partial — see above.
		if(greedy_order_deadline_passed(deadline)
			|| greedy_order_budget_spent(touch_budget, start_touches))
			return ArrayIDIDFunc();
