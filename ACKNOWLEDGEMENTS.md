...
|---|---|---|
| **FlowCutter** | Michael Hamann, Ben Strasser | Graph bisection; the treewidth solver behind `goatd::flowcutter`, and the separator search `goatd::flowcutter_rs` ports |
| **sharpSAT-TD** | Tuukka Korhonen, Matti Järvisalo | The graph and bitset layer the FlowCutter driver is built on |
| **treedecomp** | Kenji Hashimoto and the treedecomp authors | The `TreeDecomposition` representation and its definitions |
...
- Michael Abseher, Nysret Musliu, Stefan Woltran. *htd — A Free, Open-Source Framework for (Customized) Tree Decompositions and Beyond.* CPAIOR 2017. The fill-only min-fill with sampled tie-breaking that the sampled orders follow.
...
- Sebastian Schlag, Vitali Henne, Tobias Heuer, Henning Meyerhenke, Peter Sanders, Christian Schulz. *k-way Hypergraph Partitioning via n-Level Recursive Bisection.* ALENEX 2016. KaHyPar's n-level scheme, which the multilevel bisectors follow.
